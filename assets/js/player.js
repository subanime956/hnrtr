...
document.querySelectorAll(".rtr-ep[data-src]").forEach(ep => {

  ep.addEventListener("click", function(){

    const url = this.dataset.src;

    // Si no tiene URL, no hacer nada.
    // Esto permite que el episodio actual tenga data-src=""
    if(!url || !url.trim()){
      return;
    }

    // Ir a la página del episodio
